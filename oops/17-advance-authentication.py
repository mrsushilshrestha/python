...
def input_credential():
    #take a username and password from user username: mrsushilshrestha password: sushil@123
...
class Computer_shop:
    ...
    def __init__(self,path): #make a constructor
        self.path =path
...
def main():
    ...
    while command=="y":
        ...
        elif option=='3': #for update data
            username, password = input_credential()

            @authenticator
            def update_data(username, password):
                print("You Click Update Option...")
                old_name = input("Enter the Old Name Of Customer: ")
                update_name = input("Enter the New Name Of Customer: ")    
                computer_shop_obj.update_function(old_name, update_name)
            
            # Call the update_data function after authentication
